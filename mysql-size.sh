...
echo;
printf "%-11s  %-11s  %-11s  %-11s  %s\n" " On Disk   " " MySQLData " " Size Diff " " DataFree  " " Database Name  "
printf "%-11s  %-11s  %-11s  %-11s  %s\n" "-----------" "-----------" "-----------" "-----------" "---------------------------------------------------"

## Get list of databases
databases=$(mysql -h "$dbhost" --batch --skip-column-names -e 'show databases' | grep -v '^performance_schema$')
...
for x in $databases; do
	...
	dbsize=$(mysql -h$dbhost -e"select SUM(Data_length + Index_length) FROM information_schema.TABLES WHERE TABLE_SCHEMA = \"$x\";" | tail -n1);
	if [[ $dbsize == 'NULL' ]]; then dbsize=0; fi

	free=$(mysql -h$dbhost -e"select SUM(Data_free) FROM information_schema.TABLES WHERE TABLE_SCHEMA = \"$x\";" | tail -n1);
