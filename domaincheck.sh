...
#
# FMT=" ${color}%-15s  %-15s  %-3s  %-3s  %s\n"
# printf "$FMT" " Server IP" " Live IP" "SSL" "FPM" " Domain"
# printf "$FMT" "$(dash 15)" "$(dash 15)" "---" "---" "$(dash )"
#
# echo; for x in /etc/httpd/conf.d/vhost_[^000]*.conf; do
#   D=$(basename $x .conf | cut -d_ -f2);
#   V=$(grep -i virtualhost $x | head -1 | awk '{print $2}' | cut -d: -f1);
...
#   if [[ $I != $V ]]; then color="${BRIGHT}${RED}"; else color="${GREY}"; fi;
#   printf "$FMT" "$V" "$I" "${S:- - }" "${F:- - }" "$D";
# done; echo
#
