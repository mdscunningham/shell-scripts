...
_addrule(){
if grep -q "^\#.*$(getusr)$" $CONFIG; then sed -i "s|\(^\#.*$(getusr).*$\)|\1, ${TYPE} ${CMNT}|" $CONFIG; echo
  else echo -e "\n# $(getusr) $TYPE $CMNT" >> $CONFIG; echo; fi

# Have to use | instead of / in the sed here and above so the : and / do not cause sed errors
