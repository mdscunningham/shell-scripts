...
if [[ -z $1 ]]; then
    echo "This script requires a hostname parameter."
    return 1
fi
...
if [[ -z $LIVE ]]; then
    ...
    echo "Backing up var directory ..."; tar -czf var.tar.gz var/ && chown $USERNAME. var.tar.gz
    ...
    echo "Cleaning up var backup ..."; rm var.tar.gz
fi
}
